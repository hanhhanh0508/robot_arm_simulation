...
# -*- coding: utf-8 -*-
"""
Script ĐÃ FIX V2 - Xử lý trường hợp joint states chưa đầy đủ
"""
import sys
import rospy
...
class FixedPickPlace:
    def __init__(self):
        ...
        # ✅ ĐỢI CHO ĐẾN KHI NHẬN ĐƯỢC ĐỦ JOINT STATES
        ...
        if self.current_joints is None or len(self.current_joints) < 6:
            ...
            if self.current_joints:
                rospy.logerr("❌ Chỉ nhận được %d joints: %s" % (
                    len(self.current_joints), 
                    str(list(self.current_joints.keys()))
                ))
            return
    ...
    def fix_initial_state(self):
        ...
        # ✅ CHECK: Đảm bảo có đủ 6 joints
        if len(self.current_joints) < 6:
            rospy.logerr("❌ Chỉ nhận được %d/6 joints!" % len(self.current_joints))
            missing = [n for n in self.joint_names if n not in self.current_joints]
            rospy.logerr("❌ Thiếu: %s" % str(missing))
            return False
        ...
        for name in self.joint_names:
            if name not in self.raw_joints:
                rospy.logerr("❌ Joint %s không tồn tại!" % name)
                return False
...
def main():
    try:
        ...
        print("\n" + "="*70)
        print("🤖 CHƯƠNG TRÌNH PICK & PLACE (ĐÃ FIX V2!)")
        print("="*70)
        print("✅ Tự động đợi cho đủ 6 joint states")
        print("✅ Tự động fix góc khớp không hợp lệ")
        print("✅ Normalize angles về [-π, π]")
        print("="*70)
        print("\nChương trình sẽ:")
        print("  1. Đợi nhận đủ 6 joint states")
        print("  2. Kiểm tra và fix vị trí ban đầu")
        print("  3. Tạo 2 vật cản ngẫu nhiên")
        print("  4. Di chuyển: HOME → PICK → PLACE → HOME")
        print("="*70)
